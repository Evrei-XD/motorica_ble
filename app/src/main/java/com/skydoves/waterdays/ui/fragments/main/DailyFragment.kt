      ...
      alert.setPositiveButton(getString(R.string.yes)) { _: DialogInterface, _: Int ->
        try {
          ...
          if (amount in 1..2999) {
            ...
            val drink_edited = Drink(drink.index, amount.toString() + "ед", drink.date, ContextCompat.getDrawable(context!!, CapacityDrawable.getLayout(amount))!!)
  ...
  private fun dateMoveButton(v: View) {
    when (v.id) {
      ...
      R.id.next -> if (dateCount < 0) {
        ...
        Toast.makeText(context, "этого дня ещё небыло.", Toast.LENGTH_SHORT).show()
    }
  }
  ...
  private fun addItems(date: String) {
    ...
    if (cursor != null && cursor.count > 0 && cursor.moveToLast()) {
      do {
        ...
        // add listItem
        val drink = Drink(cursor.getInt(0), "Болюс " + Integer.toString(drinkAmount) + " ед", datetime[0] + ":" + datetime[1], ContextCompat.getDrawable(context!!, mainicon)!!)
